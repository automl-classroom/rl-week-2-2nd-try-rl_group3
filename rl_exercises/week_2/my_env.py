...
class MyEnv(gym.Env):
    ...
    def __init__(
        ...
        """Initialize the observation and action space for the environment.
        ...
        self.position = 0  # start in state 0

        # spaces
        n_states = self.P.shape[0]  # state 0 and 1; in this case 2
        n_actions = 2  # 0: move to state 0, 1: move to state 1
    ...
    def step(
        ...
        reward : float
            The reward at the new position/state.
        terminated : bool
        ...
        p = float(
            self.P[self.position, action]
        )  # probability p for the selected action for a given state/position
        follow = self.rng.random() < p  # check if random number < p
        a_used = (
            action if follow else 1 - action
        )  # set a_used to action or !action ( in this case only 0 or 1 possible)

        delta = -1 if a_used == 0 else 1  # define step +/-1 for used action
        self.position = max(
            0, min(self.states[-1], self.position + delta)
        )  # define new state/position of agent

        reward = float(
            self.rewards[self.position]
        )  # define reward for new state/position
        terminated = False
        truncated = (
            self.current_steps >= self.horizon
        )  # end episode if horizon is reached
    ...
    def get_reward_per_action(self) -> np.ndarray:
        ...
        R = np.zeros((nS, nA), dtype=float)  # init R with zeros
        for s in range(
            nS
        ):  # iterate over all s,a combinations and look up reward for each case (in self.rewards)
            for a in range(nA):
                nxt = max(
                    0, min(nS - 1, s + (-1 if a == 0 else 1))
                )  # calculate which entry in self.rewards needs to be looked up
    ...
    def get_transition_matrix(
        ...
        nS, nA = len(S), len(A)
        T = np.zeros((nS, nA, nS), dtype=float)  # init T with zeros
        for s in S:
            for a in A:
...
class PartialObsWrapper(gym.Wrapper):
    ...
    def _noisy_obs(self, true_obs: int) -> int:
        ...
        """
        if self.rng.random() < self.noise:  # random number is in [0, 1]
            n = self.observation_space.n
            others = [
                s for s in range(n) if s != true_obs
            ]  # generate array with all states except the true state; example for n = 4 and true_obs = 2: others = [0, 1, 3]
            return int(
                self.rng.choice(others)
            )  # choose random state from not true states
        else:
            return int(true_obs)
